...
class Config:
    ...
    def __init__(self, L=8, beta=0.4406868, cg_method="maj", cg_factor=2):
        self.L = L
        self.beta = beta
        ...
        self.keep = 1.0
        self.train_split = 9.0 / 10.0
        self.val_split = 1.0 / 9.0
    ...
    def refresh_config(self):
        ...
        self.filepath_core = "".join(
            [
                ...
                self.activ_fcn,
                "_",
                *("".join(["n", str(dn)]) for dn in self.dense_nodes),
                self.cg_method,
                str(self.cg_factor),
                "nf",
                self.nfsym,
            ]
        ).format(self.L, self.beta)
        ...
        self.cgref_datafile = "".join(
            ["./data/L{0:d}b{1:.4e}_cg", self.cg_method, str(self.cg_factor), ".h5"]
        ).format(self.cgL, self.beta)


class MultiGPUCheckpoint(tf.keras.callbacks.Callback):
...
class ConvIsing:
    ...
    def create_cg_dataset(self, config, start_cg, end_cg):
        with h5py.File(config.datafile, "r+") as dset:
            ...
            for flipidx in range(end_cg - start_cg - 1):
                ([cgimage, cgimageflip], exp_ediff) = coarse_grain(
                    self.L, self.beta, self.cg_method, self.cgf, imagearray
                )
                ...
                if start_cg == 0 and flipidx == 0:
                    h5py_create_data_catch(
                        dset,
                        "".join(
                            ["exp_ediff1_", config.cg_method, str(config.cg_factor)]
                        ),
                        data=exp_ediff,
                    )
    ...
    def compute_exact_cg(self, config):
        self.cgimage = self.dset[
            "".join(["cgimage_", config.cg_method, str(config.cg_factor)])
        ]
        self.cgimageflip = self.dset[
            "".join(["cgimageflip_", config.cg_method, str(config.cg_factor)])
        ]
        ...
        self.cg_e = lookup_cg_e(
            self.L, self.beta, self.cgf, self.cgimage, self.cgref_ss, self.cgref_e
        )
        cg_e_flip = lookup_cg_e(
            self.L, self.beta, self.cgf, self.cgimageflip, self.cgref_ss, self.cgref_e
        )
    ...
    def run_model(self, config, freeze=False):
        train_generator = GeneratorIsing(
            self.dset, config, 1, shuffle=True, set="train"
        )
        ...
        best_weight = MultiGPUCheckpoint(
            weightfile,
            self.model_energy,
            monitor="val_loss",
            save_best_only=True,
        ...
        )  # Setting patience=config.num_epochs means the
        ...
        # Using data stream
        if config.num_gpus > 1:
            par_model = tf.keras.utils.multi_gpu_model(self.model, gpus=config.num_gpus)
            par_model.compile(loss="mean_squared_error", optimizer="Nadam")
            self.history = par_model.fit_generator(
    ...
    def compute_metrics(self, config, start_cg, end_cg):
        train_generator = GeneratorIsing(
            self.dset, config, 1, shuffle=False, set="train"
        )
        test_generator1 = GeneratorIsing(
            self.dset, config, 1, shuffle=False, set="test"
        )
        test_generator1_noflip = GeneratorIsing(
            self.dset, config, start_cg, shuffle=False, set="test", flip=False
        )
        test_generator2_noflip = GeneratorIsing(
            self.dset, config, end_cg, shuffle=False, set="test", flip=False
        )
        self.train_mse_diff = self.model.evaluate_generator(
            generator=train_generator, use_multiprocessing=False
        ).ravel()
        self.test_mse_diff = self.model.evaluate_generator(
            generator=test_generator1, use_multiprocessing=False
        ).ravel()
        ...
        nn_basis = tf.keras.Model(
            inputs=self.model_energy.layers[0].input,
            outputs=self.model_energy.layers[-2].output,
        )
        nn_fine = nn_basis.predict_generator(
            generator=test_generator1_noflip, use_multiprocessing=False
        )
        nn_coarse = nn_basis.predict_generator(
            generator=test_generator2_noflip, use_multiprocessing=False
        )

        print("Singular values (fine):", tf.eval(tf.svd(nn_fine, compute_uv=False)))
        print("Condition number of nn basis (fine):", np.linalg.cond(nn_fine))
        print("Singular values (coarse):", tf.eval(tf.svd(nn_coarse, compute_uv=False)))
        ...
        Mcc = np.matmul(nn_coarse.transpose(), nn_coarse) / nn_coarse.shape[
            0
        ] - np.matmul(coarse_avg.transpose(), coarse_avg)
        self.cc_cond = np.linalg.cond(Mcc)
        Mcf = np.matmul(nn_coarse.transpose(), nn_fine) / nn_coarse.shape[
            0
        ] - np.matmul(coarse_avg.transpose(), fine_avg)
    ...
    def compute_cg_metrics(self, config):
        ...
        self.train_pred_diff = self.model.predict_generator(
            generator=train_generator, use_multiprocessing=False
        ).ravel()
        self.test_pred_diff = self.model.predict_generator(
            generator=test_generator1, use_multiprocessing=False
        ).ravel()
        self.train_cg_mse_diff = np.mean(
            np.square(self.train_pred_diff - self.train_cg_ediff)
        )
        self.test_cg_mse_diff = np.mean(
            np.square(self.test_pred_diff - self.test_cg_ediff)
        )
    ...
    def graph_loss(self, config):
        ...
        plt.yscale("log")
        plt.plot(
            self.history.history["loss"] - np.min(self.history.history["loss"]),
            ".-",
            label="loss",
        )
        plt.plot(
            self.history.history["val_loss"] - np.min(self.history.history["val_loss"]),
            ".-",
            label="val_loss",
        )
        plt.legend()
